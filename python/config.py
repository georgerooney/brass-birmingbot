...
def get_args():
    ...
    parser.add_argument(
        "--lr", type=float, default=0.0003, help="Learning rate (default: 3e-4)"
    )
    parser.add_argument("--batch-size", type=int, default=4096, help="Minibatch size")
    parser.add_argument(
        "--n-steps", type=int, default=1024, help="Rollout steps per env"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to existing .zip model to resume from",
    )
    parser.add_argument(
        "--run-name", type=str, default=None, help="Custom name for this run"
    )
    parser.add_argument(
        "--use-transformer", action="store_true", help="Use card attention transformer"
    )
