...
class BrassEnv(gym.Env):
    ...
    def __init__(
        ...
        self.lib.BrassGetMask.argtypes = [
            ctypes.c_int32,
            ctypes.POINTER(ctypes.c_int32),
        ]
        self.lib.BrassGetMask.restype = None

        self.lib.BrassGetStateJSON.argtypes = [
            ctypes.c_int32,
            ctypes.c_char_p,
            ctypes.c_int,
        ]
        ...
        self.lib.BrassGetStepMetadataJSON.argtypes = [
            ctypes.c_int32,
            ctypes.c_char_p,
            ctypes.c_int,
        ]
    ...
    def reset(
        ...
        *,
        ...
        self.lib.BrassGetObs(
            self._env_id, self._obs_buf.ctypes.data_as(ctypes.POINTER(ctypes.c_float))
        )
        self.lib.BrassGetMask(
            self._env_id, self._mask_buf.ctypes.data_as(ctypes.POINTER(ctypes.c_int32))
        )
    ...
    def step(
        self, action: int, include_state: bool = False
        ...
        if terminated or truncated:
            if "vps" not in info:
                vps = [
                    self.lib.BrassPlayerVP(self._env_id, i)
    ...
    def _fill_diagnostic_info(self, info: dict, state_json: dict) -> None:
        ...
        info["consumed_opponent_coal"] = [
            p.get("consumed_opponent_coal", 0) for p in players
        ]
        info["consumed_opponent_iron"] = [
            p.get("consumed_opponent_iron", 0) for p in players
        ]
        info["vps"] = [
            p.get("vp_audit_industries", 0) + p.get("vp_audit_links", 0)
            for p in players
        ]
