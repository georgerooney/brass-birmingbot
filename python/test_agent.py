...
def run_eval(
    ...
    model = MaskablePPO.load(
        model_path, env=env, device="cuda" if th.cuda.is_available() else "cpu"
    )
    ...
    for ep_idx in range(num_episodes):
        ...
        trace = [
            {"player": -1, "action_name": "Game Start", "state": info.get("state")}
        ]
        done = False
        ep_reward = 0
        ...
        while not done:
            ...
            if done:
                ...
                for i in range(2):
                    ...
                    if winner_idx == -1:
                        role = "winner"  # Handle draws as winner for baseline
                ...
                for step in trace:
                    ...
                    if is_winner:
                        move_stats[act_name]["win"] += 1
                    else:
                        move_stats[act_name]["lose"] += 1
                    ...
                    if city_id != -1 and slot_idx != -1:
                        ...
                        if key not in slots_stats:
                            ...
                            industry = (
                                act_name.split("Build ")[1].split(" in ")[0]
                                if "Build " in act_name
                                else "Unknown"
                            )
                            slots_stats[key] = {
                                "built": 0,
                    ...
                    if route_id != -1:
                        ...
                        if is_winner:
                            routes_stats[r_key]["win_built"] += 1
                ...
                game_summaries.append(
                    {
                        ...
                        "trace_file": f"game_{ep_idx:02d}.json"
                        if ep_idx < trace_limit
                        else None,
                    }
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Max number of full game traces to save as JSON",
    )
    parser.add_argument(
        "--output",
        type=str,
        default="eval_data",
        help="Output directory for traces and index",
    )
    parser.add_argument(
        "--sample",
        action="store_true",
        help="Use stochastic sampling instead of deterministic best action",
    )
